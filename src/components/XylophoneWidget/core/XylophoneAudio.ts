...
export function buildPentatonicTable(count: number, base: number, octaveSpan: number): Float32Array {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const semitones = PENTATONIC[n % notesPerOctave] + 6 * oct
...
export class XylophoneAudio {
  ...
  private resumingNow = false

  private resume() {
    if (this.resumed || this.resumingNow || this.ctx?.state !== "suspended") return
    this.resumingNow = true
    void this.ctx.resume()
      .then(() => { this.resumed = true })
      .finally(() => { this.resumingNow = false })
  }
  ...
  private addGestureListeners() {
    ...
    window.addEventListener("keydown", this.onGesture)
    window.addEventListener("touchstart", this.onGesture)
    window.addEventListener("touchend", this.onGesture)
  }
  ...
  private rawBytes?: ArrayBuffer

  private destroyed = false   // add this as a class field, near your other private fields (e.g. next to `private disabled = false`)
  ...
  private async ensureContext() {
    if (this.ctx || !this.rawBytes) return
    const Ctor = window.AudioContext ?? (window as unknown as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext
    if (!Ctor) return

    this.ctx = new Ctor()   // now created inside the gesture handler → starts running, not suspended
    this.master = this.ctx.createGain()
    this.master.gain.value = 1
    this.master.connect(this.ctx.destination)
    this.buffer = await this.ctx.decodeAudioData(this.rawBytes)
  }
