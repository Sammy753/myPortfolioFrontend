...
export const XYLOPHONE = {
  ...
  spinSpeed: 0.3,
  tintWrap: 10,
  group: { scale: 0.5, rotXDeg: 190, rotZDeg: 90 },
  scroll: { sensitivity: 0.05, lerp: 1 },
} // no `as const`

export function applyResponsiveXylophoneConfig(containerWidth: number) {
  if (containerWidth < 480) {
    XYLOPHONE.count = 64
    XYLOPHONE.radius = 1
    XYLOPHONE.group.scale = 0.3
    XYLOPHONE.group.rotXDeg = 25
    XYLOPHONE.group.rotZDeg = 30
  } else if (containerWidth >= 481 && containerWidth <= 768) {
    XYLOPHONE.count = 64
    XYLOPHONE.radius = 1
    XYLOPHONE.group.scale = 0.35
    XYLOPHONE.group.rotXDeg = 25
    XYLOPHONE.group.rotZDeg = 60
  } else if (containerWidth >= 769 && containerWidth <= 1024) {
    XYLOPHONE.count = 64
    XYLOPHONE.radius = 1
    XYLOPHONE.group.scale = 0.4
    XYLOPHONE.group.rotXDeg = 25
    XYLOPHONE.group.rotZDeg = 75
  }
  else {
    XYLOPHONE.count = 64
    XYLOPHONE.radius = 1
    XYLOPHONE.group.scale = 0.5
    XYLOPHONE.group.rotXDeg = 190
    XYLOPHONE.group.rotZDeg = 90
  }
}
